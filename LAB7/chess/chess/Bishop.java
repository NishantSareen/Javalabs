...
public class Bishop extends ChessPiece
{
    ...
    {
        if(desiredRow<MIN_ROW||desiredRow>MAX_ROW){
        return false;
        }
        if(desiredColumn<MIN_COL|| desiredColumn>MAX_COL){
        return false;
        }
        if(Math.abs(desiredRow-this.getRow())==Math.abs(desiredColumn-this.getColumn())){
        return true;
        }
